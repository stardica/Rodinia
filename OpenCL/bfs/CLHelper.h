...
void _clInit()
{
   ...
   //--cambine-4: Create an OpenCL command queue    
    ...
    oclHandles.program = clCreateProgramWithBinary(oclHandles.context, 1, oclHandles.devices, &binarySize, (const unsigned char**)&programBinary, &binaryStatus, &resultCL);

    if (resultCL != CL_SUCCESS)
    {
        cerr << "InitCL()::Error: In clCreateProgramWithBinary" << endl;
    }
