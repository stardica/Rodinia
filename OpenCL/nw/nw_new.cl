...
__kernel void nw_kernel1(__global int  * reference_d, 
		   ...
		   __global int  * input_itemsets_l,
		   __global int  * reference_l,
           int cols,
           int penalty,
           int blk,
           int block_width,
	...
	for( int m = 0 ; m < BLOCK_SIZE ; m++){
	  ...
	  if ( tx <= m ){
		  ...
		  int t_index_y =  m - tx + 1;
			...
			int temp = 0;
			temp = maximum2(SCORE((t_index_y-1), (t_index_x-1)) + REF((t_index_y-1), (t_index_x-1)),
					SCORE((t_index_y),   (t_index_x-1)) - (penalty));
		  SCORE(t_index_y, t_index_x) = maximum2(temp, SCORE((t_index_y-1), (t_index_x)) - (penalty));
	  }
	  barrier(CLK_GLOBAL_MEM_FENCE);
    }
