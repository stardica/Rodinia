...
int main(int argc, char** argv)
{
	...
	cl.gwSize(rows * cols);
	printf("Global work group size set\n");
	...
	cl_mem d_gpuWall = clCreateBuffer(cl.ctxt(), CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(cl_int)*(size-cols), (data + cols), NULL);
	printf("Input Buffers created\n");

	cl_mem d_gpuResult[2];
	d_gpuResult[0] = clCreateBuffer(cl.ctxt(), CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, sizeof(cl_int)*cols, data, NULL);
	...
	cl_mem d_outputBuffer = clCreateBuffer(cl.ctxt(), CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, sizeof(cl_int)*16384, h_outputBuffer, NULL);
	...
	for (int t = 0; t < rows - 1; t += pyramid_height)
	{
		...
		printf("Kernel Arguments are set\n");
		cl.launch(kn);
