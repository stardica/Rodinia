...
void OpenCL::launch(string toLaunch)
{
	...
	ret = clEnqueueNDRangeKernel(command_queue, kernelArray[toLaunch], 1, NULL, &gwsize, &lwsize, 0, NULL, NULL);
	
	if (ret != CL_SUCCESS)
	{
		printf("\nError attempting to launch %s. Error in clEnqueueNDRangeKernel with error code %i\n\n", toLaunch.c_str(), ret);
		exit(1);
	}
}
...
{
	cl_int err;
	cl_kernel kernel = clCreateKernel(this->program, kernelName.c_str(), &err);
	...
	printf("Using %zu for local work size. global size = %zu\n", lwsize,gwsize);
	if (lwsize > gwsize)
	{
		lwsize = gwsize;
		printf("Using %zu for local work size. global size = %zu\n", lwsize,gwsize);
...
void OpenCL::buildKernel()
{
	...
	program = clCreateProgramWithBinary(context, 1, &device_id[0], &binarySize, (const unsigned char**)&programBinary, &binaryStatus, &err);
...
void OpenCL::getDevices(cl_device_type deviceType)
{
	...
	if (VERBOSE)
	{
		...
		for (int i = 0; i < devices_n; i++)
		{
			...
			clGetDeviceInfo(device_id[i], CL_DEVICE_LOCAL_MEM_SIZE, sizeof(buf_ulong), &buf_ulong, NULL);
