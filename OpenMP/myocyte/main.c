...
int main(int argc, char *argv []){
	...
	for(i=0; i<workload; i++)
	{
		read(param_file, y[i][0], 91, 1, 0);
	}

	// params
	for(i=0; i<workload; i++)
	{
		read(y_file, params[i], 16, 1, 0);
	}
	...
	if(mode == 0){
		...
		#pragma omp parallel private(threads, status) shared(y, x, xmax, params, mode)
		for(i=0; i<workload; i++){
		//printf("in the loop\n");
		//fflush(stdout);
			//printf("\nThreadid:%d",omp_get_thread_num());
			status = solver(y[i], x[i], xmax, params[i], mode);
